$ErrorActionPreference='Stop'
$RepoRoot = Resolve-Path "$PSScriptRoot\.."
Set-Location $RepoRoot
$Report = Join-Path $RepoRoot "BUILD_REPORT.md"

Write-Host "=== LiveOps Sentinel Console Build ==="
Write-Host "Repository: $RepoRoot"

try {
  # vcpkg setup
  $localVcpkg = Join-Path $RepoRoot ".tools\vcpkg"
  if (!(Test-Path "$localVcpkg\.git")) {
    Write-Host "Cloning vcpkg..."
    git clone https://github.com/microsoft/vcpkg $localVcpkg
    if (Test-Path "$localVcpkg\bootstrap-vcpkg.bat") {
      Write-Host "Bootstrapping vcpkg..."
      & "$localVcpkg\bootstrap-vcpkg.bat" -disableMetrics | Out-Null
    }
  }

  $env:VCPKG_ROOT = $localVcpkg
  $toolchain = "$localVcpkg\scripts\buildsystems\vcpkg.cmake"
  $env:VCPKG_FEATURE_FLAGS='manifests,versions'

  Write-Host "VCPKG_ROOT: $localVcpkg"

  # baseline update
  if (Test-Path "$localVcpkg\.git") {
    $sha = (git -C $localVcpkg rev-parse HEAD).Trim()
    $json = Get-Content .\vcpkg.json -Raw | ConvertFrom-Json
    $json | Add-Member -Name 'builtin-baseline' -Value $sha -MemberType NoteProperty -Force
    ($json | ConvertTo-Json -Depth 10) | Set-Content .\vcpkg.json -Encoding utf8
    Write-Host "Updated baseline: $sha"
  }

  # build directory
  $ts = Get-Date -Format "yyyyMMdd_HHmmss"
  $BuildDir = "build_$ts"
  if (Test-Path "build") { 
    try { Remove-Item -Recurse -Force "build" } catch { $BuildDir = "build_$ts" }
  }
  if (!(Test-Path $BuildDir)) { New-Item -ItemType Directory -Force -Path $BuildDir | Out-Null }

  Write-Host "Build directory: $BuildDir"

  # vcpkg install
  if (Test-Path "$localVcpkg\vcpkg.exe") {
    Write-Host "Installing dependencies..."
    & "$localVcpkg\vcpkg.exe" install --triplet x64-windows
    if ($LASTEXITCODE -ne 0) {
      throw "vcpkg install failed with exit code $LASTEXITCODE"
    }
  } else {
    throw "vcpkg.exe not found at $localVcpkg\vcpkg.exe"
  }

  # Configure
  Write-Host "Configuring CMake..."
  $cfgArgs = @(
    "-S", ".",
    "-B", $BuildDir,
    "-G", "Visual Studio 17 2022",
    "-A", "x64",
    "-DCMAKE_TOOLCHAIN_FILE=`"$toolchain`"",
    "-DDISABLE_GUI=ON",
    "-DENABLE_OBS=OFF",
    "-DCMAKE_BUILD_TYPE=Release"
  )
  & cmake @cfgArgs
  if ($LASTEXITCODE -ne 0) {
    throw "CMake configuration failed with exit code $LASTEXITCODE"
  }

  # Build
  Write-Host "Building..."
  & cmake --build $BuildDir --config Release --parallel
  if ($LASTEXITCODE -ne 0) {
    throw "Build failed with exit code $LASTEXITCODE"
  }

  # Find and copy exe
  $exe = Get-ChildItem $BuildDir -Recurse -Filter "liveops_sentinel.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
  if (!$exe) {
    throw "liveops_sentinel.exe not found after build"
  }

  $outDir = Join-Path $RepoRoot "out"
  New-Item -ItemType Directory -Force -Path $outDir | Out-Null
  Copy-Item -Force $exe.FullName (Join-Path $outDir "liveops_sentinel.exe")
  Write-Host "SUCCESS: Executable copied to out\liveops_sentinel.exe"

  # Quick test
  $outExe = Join-Path $outDir "liveops_sentinel.exe"
  if (Test-Path $outExe) {
    Write-Host "Testing executable..."
    try {
      $proc = Start-Process -FilePath $outExe -NoNewWindow -PassThru
      Start-Sleep -Seconds 3
      if (!$proc.HasExited) { 
        $proc.Kill()
        Write-Host "Executable runs successfully (killed after 3s)"
      }
    } catch {
      Write-Host "Executable test failed: $_"
    }
  }

  Write-Host "BUILD SUCCESSFUL!"
  $exitCode = 0

} catch {
  Write-Host "BUILD FAILED: $($_.Exception.Message)" -ForegroundColor Red
  Write-Host "Error details: $($_.Exception)" -ForegroundColor Red
  $exitCode = 1
}

# Report
$reportContent = @"
BuildDir: $BuildDir
Toolchain: $toolchain
VCPKG_ROOT: $localVcpkg
Time: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
ExitCode: $exitCode
Status: $(if ($exitCode -eq 0) { 'SUCCESS' } else { 'FAILED' })
"@

$reportContent | Out-File -Encoding utf8 $Report
Write-Host "`n[DONE] Build report: $Report"

exit $exitCode
